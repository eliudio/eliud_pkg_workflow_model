...
typedef AssignmentWidgetProvider = Function(AssignmentModel? value);

class AssignmentListWidget extends StatefulWidget with HasFab {
  final AppModel app;
  final BackgroundModel? listBackground;
  final AssignmentWidgetProvider? widgetProvider;
  final bool? readOnly;
  final String? form;
  //final AssignmentListWidgetState? state;
  ...
  @override
  AssignmentListWidgetState createState() {
    return AssignmentListWidgetState();
  }

  @override
  Widget? fab(BuildContext context) {
    if ((readOnly != null) && readOnly!) return null;
    var state = AssignmentListWidgetState();
    return state.fab(
      context,
    );
  }
...
class AssignmentListWidgetState extends State<AssignmentListWidget> {
              ...
              Navigator.of(context).push(
                pageRouteBuilder(widget.app,
                    page: BlocProvider.value(
                        value: BlocProvider.of<AssignmentListBloc>(context),
  ...
  Widget build(BuildContext context) {
    return Apis.apis().getCoreApi().buildWhenAccessDetermined(widget.app,
        (context) {
      return BlocBuilder<AssignmentListBloc, AssignmentListState>(
...
//            app: accessState.app,
  ...
  Widget? getForm(value, action) {
    if (widget.form == null) {
      return AssignmentForm(app: widget.app, value: value, formAction: action);
    } else {
      return null;
    }
  }
...
class AssignmentListItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Dismissible(
      key: Key('__Assignment_item_${value.documentID}'),
